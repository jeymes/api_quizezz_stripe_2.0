...
export const webhookHandlerTeste = async (req: any, res: any) => {
    let event;
    ...
    if (!process.env.STRIPE_ENDPOINT_SECRET_TESTE) {
        console.error('A chave STRIPE_ENDPOINT_SECRET_TESTE não está configurada.');
        return res.status(500).json({ error: 'STRIPE_ENDPOINT_SECRET_TESTE não está configurada' });
    }
    ...
    try {
        event = stripe.webhooks.constructEvent(
            (req as any).rawBody,
            stripeSignature,
            process.env.STRIPE_ENDPOINT_SECRET_TESTE as string
        );
